	...
	_deckScale(sf::Vector2f(1.f, 1.f))
{
	setDeck(deck);
	setRendering(true);
	for (int i = 0; i <= _deck->top_index(); i++) {
		_cardRenders.push_back(cardRenderer(*_cardAtlas, &(_deck->get_card(i)), _deckScale , _deckRotation, position));
	}
}
	...
	_deckScale(sf::Vector2f(1.f, 1.f))
{
	...
	setSpacing(spacing);
	setRendering(true);
	TVector<sf::Vector2f> coords = getCoordinatesVectorFromAngle(_deck->top_index() + 1, _startPos, _angle, _spacing);
	for (int i = 0; i < coords.size(); i++) {
		_cardRenders.push_back(cardRenderer(*_cardAtlas, &(_deck->get_card(i)), 1.f, sf::Angle(sf::degrees(0)), coords[i]));
	}
	...
	_deckScale(sf::Vector2f(1.f, 1.f))
{
	...
	_spacing = length / (_deck->top_index());
	TVector<sf::Vector2f> coords = getCoordinatesVectorFromPosition(_deck->top_index() + 1, _startPos, _endPos);
	for (int i = 0; i < coords.size(); i++) {
		_cardRenders.push_back(cardRenderer(*_cardAtlas, &(_deck->get_card(i)), 1.f, sf::Angle(sf::degrees(0)), coords[i]));
	}
}
...
void deckRenderer::setCardAtlas(sf::Texture& cardAtlas) {
	_cardAtlas = &cardAtlas;
	for (int i = 0; i <= _deck->top_index(); i++) _cardRenders[i].setCardAtlas(*_cardAtlas);
}
...
void deckRenderer::setRendering(bool isRendering) {
	if (_deck->is_empty() || _deck == nullptr || _cardAtlas == nullptr) _isRendering = false;
...
void deckRenderer::update() {
	...
	if (_deck->top_index() == 0) {
		_endPos = _startPos;
		_cardRenders.push_back(cardRenderer(*_cardAtlas, &(_deck->get_card(0)), _deckScale, _deckRotation, _startPos));
	}
	else {
		...
		for (int i = 0; i <= _deck->top_index(); i++) {
			...
			_cardRenders.push_back(cardRenderer(*_cardAtlas, &(_deck->get_card(i)), _deckScale, _deckRotation, sf::Vector2f(_startPos.x + distance * dx, _startPos.y + distance * dy)));
		}
		float totalDistance = (_deck->top_index()) * _spacing;
